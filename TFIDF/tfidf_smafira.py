...
# ── Funções de leitura ────────────────────────────────────────────────────────
def load_facets(pmid):
    filepath = os.path.join(FACETS_DIR, f"{pmid}_intersect.txt")
    facets = []
...
def fetch_abstract(pmid):
    try:
        ...
        text = handle.read()
        handle.close()
        time.sleep(0.4)  # respeitar limite da API
        return text
...
# ── Pipeline principal ────────────────────────────────────────────────────────
print("=" * 60)
print("TF-IDF SMAFIRA-c — Facets vs Candidatos")
print("=" * 60)
...
for seed_pmid in PMIDS_SEEDS:
    ...
    # 1. Carregar facets do seed
    seed_text = load_facets(seed_pmid)
    print(f"  Facets: {len(seed_text.split())} palavras")
    ...
    # 3. Baixar abstracts dos candidatos
    print(f"  A baixar abstracts do PubMed...")
    candidate_texts = []
